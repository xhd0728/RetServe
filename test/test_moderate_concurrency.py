...
class ModerateConcurrencyTester:
        ...
        for i, (response_time_ms, result) in enumerate(results):
            if response_time_ms < 0 or "error" in result:
                failed_requests.append(
                    {"request_id": i, "error": result.get("error", "Unknown error")}
                )
            else:
    ...
    async def progressive_load_test(
        self,
        queries: List[str],
        concurrency_start: int = 5,
        concurrency_end: int = 30,
        step: int = 5,
    ) -> Dict[str, Any]:
        """
        ...
        while current_concurrency <= concurrency_end:
            ...
            results.append({"concurrency": current_concurrency, **result})
            ...
            if (
                ...
                and current_concurrency > concurrency_start
            ):
                print(
                    f"  Warning: Low success rate ({metrics['success_rate']:.2%}), stopping progressive tests"
                )
                break
...
def print_test_summary(test_results: Dict[str, Any]) -> None:
    ...
    print(
        f"Test range: {config['concurrency_start']} to {config['concurrency_end']} concurrent requests"
    )
    ...
    for result in test_results["results"]:
        metrics = result["metrics"]
        print(
            f"{metrics['total_requests']:<12} {metrics['success_rate']:<11.2%} "
            f"{metrics['avg_response_time_ms']:<14.2f} {metrics['throughput']:<14.2f} "
            f"{metrics['total_time_ms']:<14.2f}"
        )

    successful_runs = [
        r for r in test_results["results"] if r["metrics"]["success_rate"] > 0.5
    ]
    if successful_runs:
    ...
    total_failures = sum(
        r["metrics"]["failed_requests"] for r in test_results["results"]
    )
    if total_failures > 0:
        ...
        for result in test_results["results"]:
            if result["metrics"]["failed_requests"] > 0:
                print(
                    f"  Concurrency {result['concurrency']}: {result['metrics']['failed_requests']} failures"
                )
...
async def main():
    ...
    try:
        ...
        test_results = await tester.progressive_load_test(
            test_queries, concurrency_start=5, concurrency_end=30, step=5
        )

...
class ConcurrentRetrievalTester:
    ...
    async def single_search(
        self, session: aiohttp.ClientSession, query: str, topk: int = 5
        ...
        payload = {"queries": [formatted_query], "topk": topk}
    ...
    async def concurrent_search(
        self, queries: List[str], concurrency: int, topk: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                failed_requests.append({"request_id": i, "error": str(result)})
            else:
        ...
        metrics = {
            ...
            "avg_time_per_request_ms": statistics.mean(response_times)
            if response_times
            else 0,
            "min_time_ms": min(response_times) if response_times else 0,
            "max_time_ms": max(response_times) if response_times else 0,
            "p95_time_ms": sorted(response_times)[int(len(response_times) * 0.95)]
            if len(response_times) > 20
            else 0,
            "throughput": concurrency / (total_time_ms / 1000)
            if total_time_ms > 0
            else 0,
        }

        return {"metrics": metrics, "failed_requests": failed_requests}

    async def load_test(
        self, queries: List[str], concurrency_levels: List[int], topk: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        for concurrency in sorted(concurrency_levels):
            ...
            load_test_results.append({"concurrency": concurrency, **result})
...
def print_load_test_summary(load_test_results: Dict[str, Any]) -> None:
    ...
    print(
        f"Tested concurrency levels: {load_test_results['test_config']['concurrency_levels']}"
    )
    ...
    print("\n" + "=" * 70)
    print(
        f"{'Concurrency':<12} {'Success Rate':<12} {'Avg Time (ms)':<15} {'Throughput':<15} {'Total Time (ms)':<15}"
    )
    print("-" * 70)
    ...
    for result in load_test_results["results"]:
        metrics = result["metrics"]
        print(
            f"{metrics['total_requests']:<12} {metrics['success_rate']:<11.2%} {metrics['avg_time_per_request_ms']:<14.2f} {metrics['throughput']:<14.2f} {metrics['total_time_ms']:<14.2f}"
        )

    best_throughput = max(
        result["metrics"]["throughput"] for result in load_test_results["results"]
    )
    best_result = next(
        r
        for r in load_test_results["results"]
        if r["metrics"]["throughput"] == best_throughput
    )

    print("\n" + "=" * 70)
    print(
        f"Best throughput: {best_throughput:.2f} requests/sec at concurrency {best_result['concurrency']}"
    )

    total_failures = sum(
        len(r["failed_requests"]) for r in load_test_results["results"]
    )
    if total_failures > 0:
        ...
        for result in load_test_results["results"]:
            if result["failed_requests"]:
                print(
                    f"  Concurrency {result['concurrency']}: {len(result['failed_requests'])} failures"
                )
                for failure in result["failed_requests"][:3]:  # Show first 3 failures
                    print(
                        f"    - Request {failure['request_id']}: {failure['error'][:100]}..."
                    )
...
async def main():
    ...
    try:
        ...
        load_test_results = await tester.load_test(
            test_queries, concurrency_levels, topk
        )

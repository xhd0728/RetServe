...
class FinalPerformanceTester:
    ...
    async def concurrent_test(
        self, queries: List[str], concurrency: int, iterations: int = 5, topk: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        async with aiohttp.ClientSession() as session:
            for i in range(iterations):
                ...
                iteration_success_rate = (
                    iteration_success / iteration_requests
                    if iteration_requests > 0
                    else 0
                )
                iteration_avg_time = (
                    sum(iteration_response_times) / len(iteration_response_times)
                    if iteration_response_times
                    else 0
                )
                iteration_throughput = (
                    iteration_requests / total_time if total_time > 0 else 0
                )
        ...
        return {
            ...
            "total_throughput": total_requests / (total_time * iterations)
            if total_time > 0
            else 0,
        }
...
""")
        ...
        print("Testing with 50 concurrent requests, 3 iterations...")
        result_50 = await self.concurrent_test(
            test_queries, concurrency=50, iterations=3, topk=5
        )

        print("\n" + "=" * 70)
        print("50 concurrent requests summary:")
        ...
        result_100 = await self.concurrent_test(
            test_queries, concurrency=100, iterations=3, topk=5
        )

        print("\n" + "=" * 70)
        print("100 concurrent requests summary:")
        ...
        if result_100["success_rate"] > 0.8:
            ...
            print(
                f"✓ Service can handle {max(50, int(result_100['total_success'] / 3))} concurrent requests reliably"
            )

        print("\nOverall performance metrics:")
        print(
            f"  Best success rate: {max(result_50['success_rate'], result_100['success_rate']):.2%}"
        )
        print(
            f"  Best avg response time: {min(result_50['avg_response_time_ms'], result_100['avg_response_time_ms']):.2f} ms"
        )

        return {"50_concurrent": result_50, "100_concurrent": result_100}

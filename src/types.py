...
class SearchRequest(BaseModel):
    ...
    top_k: int = Field(
        default=5, gt=0, alias="topk", description="Number of top results per query"
    )
...
class SearchResponse(BaseModel):
    ...
    scores: list[list[float]] = Field(..., description="Scores for each query")
...
class HealthResponse(BaseModel):
    ...
    embedding_url: str = Field(
        ..., alias="emb_url", description="Embedding service URL"
    )
    embedding_model: str = Field(
        ..., alias="emb_model", description="Embedding model name"
    )
    gpu_enabled: bool = Field(
        ..., alias="use_gpu", description="GPU acceleration status"
    )
